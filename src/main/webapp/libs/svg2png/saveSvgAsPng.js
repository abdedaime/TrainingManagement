(function() {
  ...
  function inlineImages(el, callback) {
    ...
    var left = images.length;
      //Adil
    var dontProcessImages = true;
    if (left == 0 || dontProcessImages) {
      callback();
    }
  ...
  out$.saveSvgAsPng = function(el, name, options) {
    ...
    out$.svgAsDataUri(el, options, function(uri) {
      ...
      image.onload = function() {
        var canvas = document.createElement('canvas');
        canvas.width = image.width ? image.width : $(el).attr("width");
        canvas.height = image.height ? image.height : $(el).attr("height");
        download(canvas, name,image)
      }
    });
    ...
    function download(canvas, filename,image) {
        download_in_ie(canvas, filename) || download_with_link(canvas, filename,image);
    }
    ...
    function download_with_link(canvas, filename,image) {
